...
export const ItineraryOutput = defineSchema(
  "ItineraryOutput",
  z.array(
    z.object({
      place: z.string(),
          ...
          planForDay: z.array(
            z.object({
              activityRef: z
                .string()
                .describe(
                  "the reference value for the activity - this comes from the available activities JSON. If no value is present use a ref value of `restaurant`."
                ),
...
// SIMPLE RAG + TOOLS FLOW ABOVE =========

// MULTISTEP + LOOP FLOW BELLOW =========

// export const generateImageDescription = async (imageUrls?: string[]) => {
...
export const itineraryGenerator2 = defineFlow(
  ...
  async (userInputs) => {
    ...
    const possibleDestinations = await run("Suggest Destinations", async () => {
      ...
      const suggestDestinationsAgentPrompt = await prompt(
        "suggestDestinationsWithContextAgent"
      );
      ...
      const { destinations } = result.output() as {
        destinations: Destination[];
      };

      // #region : Clean Up images
      destinations.forEach((dest) => {
        const doc1 = contextDestinations.find(
          (doc) => doc.toJSON().metadata!.ref === dest.ref
        );
        if (doc1) {
    ...
    possibleDestinations.forEach((dest) => {
      const loc0 = run(
        `Plan Itinerary for Destination: ` + dest.ref,
        (): Promise<unknown> => {
    ...
    const itineraries = await run(
      "Finally Merge all Results into Itinerary",
      async () => {
        const results = await Promise.all(destDetails);
        const itineraries = {
          itineraries: [...(results as ItineraryGeneratorOutput[])],
        };
        return itineraries;
      }
    );
